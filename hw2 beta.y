...
else_statement: block '}'   {fprintf(javacode,"\tL%d:\n",label_stack[--label_top]+1);}
			...
			|  	block '}' ELSE
				...
				block '}'  {fprintf(javacode,"\tL%d:\n",label_stack[--label_top]+1);fprintf(javacode,"\t\tgetstatic java.io.PrintStream java.lang.System.out\n");
...
bool_expression: expression EQ expression				// the bool check ex: a==b reduce to bool_expression;
		...
		|   TRUE 	{$$.tokentype=4;$$.boolval=true;if(is_local==1){printtab();fprintf(javacode,"iconst_1\n");}Trace("< TRUE reduce to bool_expression >\n");}
		|   FALSE 	{$$.tokentype=4;$$.boolval=false;if(is_local==1){printtab();fprintf(javacode,"iconst_0\n");}Trace("< FALSE reduce to bool_expression >\n");}
		|   NAME
		{
			$$.tokentype=4;
			int choose = 0;
			...
			$$.boolval = thetab[choose][index].thebool;
			if(thetab[choose][index].thestate)
		}
		;
